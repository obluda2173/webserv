		...
		for (std::vector<struct pollfd>::iterator it = _pfds.begin(); it != _pfds.end();) {
			...
			if (it->revents & POLLOUT) {
				if (count == 0) {
					...
					// I think we need a new struct to store the response
					// the first step will be to read the uri, and find out what kind of uri we receive, and then fill uri full

					std::ifstream file(newRequest.uri.c_str());
					...
					response = "HTTP/1.1 200 OK\r\n"
							   ...
							   "\r\n" +
							   fileContent;
					// Reset count to allow sending the response again
					count = 0;
				}
