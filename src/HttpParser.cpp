...
bool	isValidContentType(const std::string& str) {
	...
	if (slash == std::string::npos || slash == 0 || slash == str.size() - 1) //the char '/' must be in the middle of the string
		return false;
	std::string type = str.substr(0, slash);
	if (type != "application" && type != "text" && type != "image" &&
		type != "audio" && type != "video" && type != "multipart" &&
		type != "font" && type != "model" && type != "message" &&
		type != "example") {
		return false;
	}
...
bool	isValidAccept(const std::string& str) {
	...
	for (size_t i = 0; i < tokens.size(); i++) {
		...
		if (slash == std::string::npos || slash == 0 || slash == token2.size() - 1) //the char '/' must be in the middle of the string
			return false;
...
bool	isValidAcceptEncoding(const std::string& str) {
	...
	for (size_t i = 0; i < tokens.size(); i++) {
		std::string token2 = tokens[i];
		if (token2 != "gzip" && token2 != "deflate" && token2 != "br" &&
			token2 != "compress" && token2 != "identity") {
			return false;
		}
		if (checkQValue(token2) == false) {
			return false;
...
bool	isValidAcceptLanguage(const std::string& str) {
	...
	for (size_t i = 0; i < tokens.size(); i++) {
		...
		if (dash != std::string::npos && (dash == 0 || dash == token2.size() - 1)) //the char '-' should be in the middle of the string(if appears)
			return false;
...
bool	isValidCookie(std::string str) {
	...
	for (size_t i = 0; i < tokens.size(); i++) {
		...
		if (equal == std::string::npos || equal == 0 || equal == token2.size() - 1) //the char '=' must be in the middle of the string
			return false;
...
bool	isValidRange(const std::string& str) {
	...
	for (size_t i = 0; i < tokens.size(); i++) {
		std::string token2 = tokens[i];
		size_t dash = token2.find('-');
		if (dash == std::string::npos || dash == 0 || dash == token2.size() - 1) //the char '-' must be in the middle of the string
			return false;
