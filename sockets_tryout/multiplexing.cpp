...
void fun() {
    ...
    pfds.push_back(
        {_server_fd, POLLIN,
         0}); // Initial file descriptor setup to listen for new connections
            ...
            if (pfds[i].revents & POLLIN) {
                if (pfds[i].fd == _server_fd) {
                    ...
                    int new_socket = accept(
                        _server_fd, (struct sockaddr *)&_address, &_addrlen);
                    ...
                    if (bytes_read > 0) {
                        ...
                        pfds[i].events = pfds[i].events |
                                         POLLOUT; // Add POLLOUT to event flags
                    } else {
