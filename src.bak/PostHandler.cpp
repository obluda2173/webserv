...
//             conn->uploadCtx.file = new std::ofstream(path.c_str(), std::ios::binary | std::ios::app);
//             if (!conn->uploadCtx.file->is_open()) {
//                 std::cerr << "Failed to open file" << std::endl;
//                 return;
//             }
//             std::map< std::string, std::string >::const_iterator it = request.headers.find("content-length");
//             conn->uploadCtx.contentLength = atoi(it->second.c_str());
//         }
//         if (conn->uploadCtx.bytesUploaded + conn->_readBufUsedSize <= conn->uploadCtx.contentLength) {
//             conn->uploadCtx.file->write(conn->getReadBuf().c_str(), conn->_readBufUsedSize);
//             conn->uploadCtx.bytesUploaded += conn->_readBufUsedSize;
//         } else {
//             conn->uploadCtx.file->write(conn->getReadBuf().c_str(),
//                                         conn->uploadCtx.contentLength - conn->uploadCtx.bytesUploaded);
//             conn->uploadCtx.bytesUploaded = conn->uploadCtx.contentLength;
//         }
//     } else if (request.headers.find("transfer-encoding") !=
//                request.headers.end()) { // we might need to change this in the future
...
//             if (endptr == size_str.c_str() || chunk_len < 0)
//                 break;
...
//             if (i + (size_t)chunk_len > body.size())
//                 break;
//             conn->uploadCtx.file->write(body.data() + i, chunk_len);
...
//             _setResponse(resp, code, message, _getMimeType(errorPagePath), fileStat.st_size,
...
// void PostHandler::_setResponse(HttpResponse& resp, int statusCode, const std::string& statusMessage,
//                                const std::string& contentType, size_t contentLength, IBodyProvider* bodyProvider) {
